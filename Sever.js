...
// NOTE: local /uploads static serving removed — images now live on Cloudinary.
...
// ─── Cloudinary storage (replaces old multer.diskStorage) ───────────────────── ← NEW
// All uploaded images go straight to Cloudinary instead of local disk.
// req.file.path will contain the final hosted (CDN) URL.
const storage = new CloudinaryStorage({
  cloudinary,
  params: {
    folder: "smartpos",                                    // Cloudinary folder name
...
app.put("/categories/:id", upload.single("image"), async (req, res) => {
  try {
    ...
    const image       = req.file ? req.file.path : existing[0].image;  // ← Cloudinary URL
